...
static int
test_dwt(int *array, int *ref, int border[2][2], int decomp_levels, int type, int max_diff) {
    int ret, j;
    ...
    for (j = 0; j < MAX_W * MAX_W; j++) {
        if (FFABS(array[j] - ref[j]) > max_diff) {
            fprintf(stderr, "missmatch at %d (%d != %d) decomp:%d border %d %d %d %d\n",
                    j, array[j], ref[j], decomp_levels, border[0][0], border[0][1], border[1][0],
                    border[1][1]);
            return 2;
        }
    ...
    printf("%s, decomp:%2d border %3d %3d %3d %3d milli-err2:%9"
    PRId64
    "\n",
...
static int
test_dwtf(float *array, float *ref, int border[2][2], int decomp_levels, float max_diff) {
    int ret, j;
    DWTContext s1 = {{{0}}}, *s = &s1;
    double err2 = 0;
...
int main(void) {
    ...
    for (i = 0; i < 100; i++) {
        ...
        if (ret)
            return ret;
        ret = test_dwt(array, ref, border, decomp_levels, FF_DWT97_INT,

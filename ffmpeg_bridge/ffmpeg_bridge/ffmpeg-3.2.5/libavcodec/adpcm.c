...
adpcm_ima_wav_expand_nibble(ADPCMChannelStatus *c, GetBitContext *gb, int bps) {
...
static inline int16_t
adpcm_sbpro_expand_nibble(ADPCMChannelStatus *c, int8_t nibble, int size, int shift) {
    int sign, delta, diff;
...
static void
adpcm_swf_decode(AVCodecContext *avctx, const uint8_t *buf, int buf_size, int16_t *samples) {
    ADPCMDecodeContext *c = avctx->priv_data;
    GetBitContext gb;
    const int *table;
    ...
    while (get_bits_count(&gb) <= size - 22 * avctx->channels) {
        ...
        for (count = 0;
             get_bits_count(&gb) <= size - nb_bits * avctx->channels && count < 4095; count++) {
            int i;
    ...
    switch (avctx->codec->id) {
        ...
        case AV_CODEC_ID_ADPCM_IMA_WAV:
            ...
            if (avctx->bits_per_coded_sample != 4) {
                int samples_per_block = ff_adpcm_ima_block_samples[avctx->bits_per_coded_sample -
                                                                   2];
        ...
        case AV_CODEC_ID_ADPCM_MTAF:
            for (channel = 0; channel < avctx->channels; channel += 2) {
                ...
                for (n = 0; n < nb_samples; n += 2) {
                    ...
                    samples_p[channel][n + 1] = adpcm_mtaf_expand_nibble(&c->status[channel],
                                                                         v >> 4);
                }
                for (n = 0; n < nb_samples; n += 2) {
                    int v = bytestream2_get_byteu(&gb);
                    samples_p[channel + 1][n] = adpcm_mtaf_expand_nibble(&c->status[channel + 1],
                                                                         v & 0x0F);
                    samples_p[channel + 1][n + 1] = adpcm_mtaf_expand_nibble(
                            &c->status[channel + 1], v >> 4);
                }
            }
            break;
        case AV_CODEC_ID_ADPCM_IMA_DK4:
    ...
    if (decode_top_nibble_next) { \
        ...
        case AV_CODEC_ID_ADPCM_IMA_RAD:
            ...
            for (n = 0; n < nb_samples / 2; n++) {
                ...
                for (channel = 0; channel < avctx->channels; channel++) {
                    *samples++ = adpcm_ima_expand_nibble(&c->status[channel], byte[channel] & 0x0F,
                                                         3);
                }
                for (channel = 0; channel < avctx->channels; channel++) {
                    *samples++ = adpcm_ima_expand_nibble(&c->status[channel], byte[channel] >> 4,
                                                         3);
                }
            }
            break;
                ...
                for (i = 0; i < (nb_samples + 13) / 14; i++) {
                    ...
                    for (n = 0; n < 14 && (i * 14 + n < nb_samples); n++) {
                        ...
                        sampledat = ((c->status[ch].sample1 * factor1
                                      + c->status[ch].sample2 * factor2) >> 11) +
                                    (sampledat << exp);
                        *samples = av_clip_int16(sampledat);
                ...
                for (i = 0; i < nb_samples / 28; i++) {
                    ...
                    for (n = 0; n < 28; n++) {
                        ...
                        if (flag < 0x07) {
                            ...
                            scale = scale << 12;
                            sample = (int) ((scale >> shift) + (c->status[channel].sample1 *
                                                                xa_adpcm_table[filter][0] +
                                                                c->status[channel].sample2 *
                                                                xa_adpcm_table[filter][1]) / 64);
                        }

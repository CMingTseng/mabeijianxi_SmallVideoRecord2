...
void ff_atrac3p_power_compensation(Atrac3pChanUnitCtx *ctx, int ch_index,
    ...
    if (ctx->channels[ch_index ^ swap_ch].power_levs[subband_to_powgrp[sb]] ==
    ...
    grp_lev = pwc_levs[ctx->channels[ch_index ^ swap_ch].power_levs[subband_to_powgrp[sb]]] /
              (1 << gcv);
...
void ff_atrac3p_imdct(AVFloatDSPContext *fdsp, FFTContext *mdct_ctx, float *pIn,
    ...
    if (sb & 1)
        for (i = 0; i < ATRAC3P_SUBBAND_SAMPLES / 2; i++)
            FFSWAP(
    float, pIn[i], pIn[ATRAC3P_SUBBAND_SAMPLES - 1 - i]);

...
typedef struct AMRContext {
    ...
    float excitation_buf[PITCH_DELAY_MAX + LP_FILTER_ORDER + 1 +
                         AMR_SUBFRAME_SIZE]; ///< current excitation and all necessary excitation history
...
static void lsf2lsp_3(AMRContext *p) {
    ...
    for (i = 0; i < LP_FILTER_ORDER; i++)
        lsf_q[i] = (lsf_r[i] + p->prev_lsf_r[i] * pred_fac[i]) * (LSF_R_FAC / 8000.0) +
                   lsf_3_mean[i] * (1.0 / 8000.0);
...
static void decode_fixed_sparse(AMRFixed *fixed_sparse, const uint16_t *pulses,
    ...
    if (mode == MODE_12k2) {
        ...
        if (mode <= MODE_5k15) {
            ...
            pulse_position[1] =
                    ((fixed_index >> 6) & 7) * 5 + pulse_subset + (pulse_subset == 3 ? 1 : 0);

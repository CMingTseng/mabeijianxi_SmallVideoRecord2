...
static void set_special_band_scalefactors(AACEncContext *s, SingleChannelElement *sce) {
    ...
    for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
        for (g = 0; g < sce->ics.num_swb; g++) {
            ...
            if (sce->band_type[w * 16 + g] == INTENSITY_BT ||
                ...
                sce->sf_idx[w * 16 + g] = av_clip(roundf(log2f(sce->is_ener[w * 16 + g]) * 2), -155,
                                                  100);
                bands++;
            } else if (sce->band_type[w * 16 + g] == NOISE_BT) {
                sce->sf_idx[w * 16 + g] = av_clip(3 + ceilf(log2f(sce->pns_ener[w * 16 + g]) * 2),
                                                  -100, 155);
                if (prevscaler_n == -255)
    ...
    for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
        for (g = 0; g < sce->ics.num_swb; g++) {
            ...
            if (sce->band_type[w * 16 + g] == INTENSITY_BT ||
                sce->band_type[w * 16 + g] == INTENSITY_BT2) {
                sce->sf_idx[w * 16 + g] = prevscaler_i = av_clip(sce->sf_idx[w * 16 + g],
                                                                 prevscaler_i - SCALE_MAX_DIFF,
                                                                 prevscaler_i + SCALE_MAX_DIFF);
            } else if (sce->band_type[w * 16 + g] == NOISE_BT) {
                sce->sf_idx[w * 16 + g] = prevscaler_n = av_clip(sce->sf_idx[w * 16 + g],
                                                                 prevscaler_n - SCALE_MAX_DIFF,
                                                                 prevscaler_n + SCALE_MAX_DIFF);
            }
        }
    }
...
static void search_for_quantizers_anmr(AVCodecContext *avctx, AACEncContext *s,
    ...
    if (q1 - q0 > 60) {
        ...
        int qnrg = av_clip_uint8(
                log2f(sqrtf(qnrgf / qcnt)) * 4 - 31 + SCALE_ONE_POS - SCALE_DIV_512);
        q1 = qnrg + 30;
        q0 = qnrg - 30;
        if (q0 < q0low) {
            q1 += q0low - q0;
    ...
    for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
        ...
        for (g = 0; g < sce->ics.num_swb; g++) {
            ...
            if (nz) {
                ...
                maxval = find_max_val(sce->ics.group_len[w], sce->ics.swb_sizes[g],
                                      s->scoefs + start);
                for (q = minscale; q < maxscale; q++) {
                    ...
                    for (w2 = 0; w2 < sce->ics.group_len[w]; w2++) {
                        ...
                        dist += quantize_band_cost(s, coefs + w2 * 128,
                                                   ...
                                                   sce->ics.swb_sizes[g],
                                                   q + q0, cb, lambda / band->threshold, INFINITY,
                                                   NULL, NULL, 0);
                    }
...
static void search_for_quantizers_fast(AVCodecContext *avctx, AACEncContext *s,
    ...
    int destbits =
            avctx->bit_rate * 1024.0 / avctx->sample_rate / avctx->channels * (lambda / 120.f);
    ...
    for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
        for (g = 0; g < sce->ics.num_swb; g++) {
            ...
            sce->sf_idx[w * 16 + g] =
                    SCALE_ONE_POS + FFMIN(log2f(uplims[w * 16 + g] / minthr) * 4, 59);
        }
    }
    ...
    for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
        ...
        for (g = 0; g < sce->ics.num_swb; g++) {
            ...
            maxvals[w * 16 + g] = find_max_val(sce->ics.group_len[w], sce->ics.swb_sizes[g],
                                               scaled);
            start += sce->ics.swb_sizes[g];
        }
    }
        ...
        do {
            ...
            for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
                ...
                for (g = 0; g < sce->ics.num_swb; g++) {
                    ...
                    if (prev != -1) {
                        bits += ff_aac_scalefactor_bits[sce->sf_idx[w * 16 + g] - prev +
                                                        SCALE_DIFF_ZERO];
                    }
                    tbits += bits;
                    start += sce->ics.swb_sizes[g];
...
static void search_for_pns(AACEncContext *s, AVCodecContext *avctx, SingleChannelElement *sce) {
    ...
    for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
        ...
        for (g = 0; g < sce->ics.num_swb; g++) {
             ...
             */
            if ((!sce->zeroes[w * 16 + g] &&
                 !ff_sfdelta_can_remove_band(sce, nextband, prev_sf, w * 16 + g)) ||
                ((sce->zeroes[w * 16 + g] || !sce->band_alt[w * 16 + g]) &&
                 sfb_energy < threshold * sqrtf(1.0f / freq_boost)) || spread < spread_threshold ||
                (!sce->zeroes[w * 16 + g] && sce->band_alt[w * 16 + g] &&
            ...
            if (sce->zeroes[w * 16 + g] || !sce->band_alt[w * 16 + g] ||
                (energy_ratio > 0.85f && energy_ratio < 1.25f && dist2 < dist1)) {
...
static void mark_pns(AACEncContext *s, AVCodecContext *avctx, SingleChannelElement *sce) {
    ...
    for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
        for (g = 0; g < sce->ics.num_swb; g++) {
             ...
             */
            sce->pns_ener[w * 16 + g] = sfb_energy;
            if (sfb_energy < threshold * sqrtf(1.5f / freq_boost) || spread < spread_threshold ||
                min_energy < pns_transient_energy_r * max_energy) {
                sce->can_pns[w * 16 + g] = 0;
            } else {
...
static void search_for_ms(AACEncContext *s, ChannelElement *cpe) {
    ...
    for (w = 0; w < sce0->ics.num_windows; w += sce0->ics.group_len[w]) {
        ...
        for (g = 0; g < sce0->ics.num_swb; g++) {
                ...
                for (sid_sf_boost = 0; sid_sf_boost < 4; sid_sf_boost++) {
                    ...
                    if (sce0->band_type[w * 16 + g] != NOISE_BT &&
                        ...
                        && (!ff_sfdelta_can_replace(sce0, nextband0, prev_mid, mididx, w * 16 + g)
                            || !ff_sfdelta_can_replace(sce1, nextband1, prev_side, sididx,
                                                       w * 16 + g))) {
                    ...
                    for (w2 = 0; w2 < sce0->ics.group_len[w]; w2++) {
                        FFPsyBand *band0 = &s->psy.ch[s->cur_channel + 0].psy_bands[(w + w2) * 16 +
                                                                                    g];
                        FFPsyBand *band1 = &s->psy.ch[s->cur_channel + 1].psy_bands[(w + w2) * 16 +
                                                                                    g];
                    ...
                    if (cpe->ms_mask[w * 16 + g]) {
                        ...
                        } else if ((sce0->band_type[w * 16 + g] != NOISE_BT) ^
                                   (sce1->band_type[w * 16 + g] != NOISE_BT)) {
            ...
            if (!sce1->zeroes[w * 16 + g] && !cpe->is_mask[w * 16 + g] &&
                sce1->band_type[w * 16 + g] < RESERVED_BT)

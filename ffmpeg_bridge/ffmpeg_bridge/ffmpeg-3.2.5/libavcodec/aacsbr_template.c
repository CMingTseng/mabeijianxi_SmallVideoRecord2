...
static int sbr_hf_calc_npatches(AACContext *ac, SpectralBandReplication *sbr) {
    ...
    do {
        ...
        sbr->patch_start_subband[sbr->num_patches] =
                sbr->k[0] - odd - sbr->patch_num_subbands[sbr->num_patches];
...
static void copy_sbr_grid(SBRData *dst, const SBRData *src) {
    ...
    memcpy(dst->bs_freq_res + 1, src->bs_freq_res + 1,
           sizeof(dst->bs_freq_res) - sizeof(*dst->bs_freq_res));
        ...
        if (ch_data->bs_df_env[i]) {
            ...
            if (ch_data->bs_freq_res[i + 1] == ch_data->bs_freq_res[i]) {
                for (j = 0; j < sbr->n[ch_data->bs_freq_res[i + 1]]; j++) {
                    ch_data->env_facs_q[i + 1][j] = ch_data->env_facs_q[i][j] +
                ...
                for (j = 0; j < sbr->n[ch_data->bs_freq_res[i + 1]]; j++) {
                    ...
                    ch_data->env_facs_q[i + 1][j] = ch_data->env_facs_q[i][k] +
                                                    delta * (get_vlc2(gb, t_huff, 9, 3) - t_lav);
                    if (ch_data->env_facs_q[i + 1][j] > 127U) {
                        av_log(ac->avctx, AV_LOG_ERROR, "env_facs_q %d is invalid\n",
                               ch_data->env_facs_q[i + 1][j]);
            ...
            ch_data->env_facs_q[i + 1][0] =
                    delta * get_bits(gb, bits); // bs_env_start_value_balance
            for (j = 1; j < sbr->n[ch_data->bs_freq_res[i + 1]]; j++) {
                ch_data->env_facs_q[i + 1][j] = ch_data->env_facs_q[i + 1][j - 1] +
        ...
        if (ch_data->bs_df_noise[i]) {
            for (j = 0; j < sbr->n_q; j++) {
                ch_data->noise_facs_q[i + 1][j] =
                        ch_data->noise_facs_q[i][j] + delta * (get_vlc2(gb, t_huff, 9, 2) - t_lav);
            ...
            ch_data->noise_facs_q[i + 1][0] = delta * get_bits(gb,
                                                               5); // bs_noise_start_value_balance or bs_noise_start_value_level
            for (j = 1; j < sbr->n_q; j++) {
                ch_data->noise_facs_q[i + 1][j] = ch_data->noise_facs_q[i + 1][j - 1] +
                                                  delta * (get_vlc2(gb, f_huff, 9, 3) - f_lav);
                if (ch_data->noise_facs_q[i + 1][j] > 30U) {
                    av_log(ac->avctx, AV_LOG_ERROR, "noise_facs_q %d is invalid\n",
                           ch_data->noise_facs_q[i + 1][j]);
...
#if 1
                *num_bits_left -= AAC_RENAME(ff_ps_read_data)(ac->avctx, gb, &sbr->ps,
                                                              *num_bits_left);
...
int AAC_RENAME(ff_decode_sbr_extension)(AACContext *ac, SpectralBandReplication *sbr,
    ...
    if (!sbr->sample_rate)
        sbr->sample_rate = 2 *
                           ac->oc[1].m4ac.sample_rate; //TODO use the nominal sample rate for arbitrary sample rate support
...
static int sbr_mapping(AACContext *ac, SpectralBandReplication *sbr,
    ...
    for (e = 0; e < ch_data->bs_num_env; e++) {
        ...
        for (i = 0; i < sbr->n[1]; i++) {
            if (ch_data->bs_add_harmonic_flag) {
                ...
                ch_data->s_indexmapped[e + 1][m_midpoint - sbr->kx[1]] =
                        ch_data->bs_add_harmonic[i] *
    ...
    memcpy(ch_data->s_indexmapped[0], ch_data->s_indexmapped[ch_data->bs_num_env],
...
static void sbr_env_estimate(AAC_FLOAT (*e_curr)[48], INTFLOAT X_high[64][40][2],
    ...
    if (sbr->bs_interpol_freq) {
        ...
        for (e = 0; e < ch_data->bs_num_env; e++) {
            ...
            int ilb = ch_data->t_env[e] * 2 + ENVELOPE_ADJUSTMENT_OFFSET;
            int iub = ch_data->t_env[e + 1] * 2 + ENVELOPE_ADJUSTMENT_OFFSET;
            const uint16_t *table = ch_data->bs_freq_res[e + 1] ? sbr->f_tablehigh
                                                                : sbr->f_tablelow;
...
void AAC_RENAME(ff_sbr_apply)(AACContext *ac, SpectralBandReplication *sbr, int id_aac,
    ...
    for (ch = 0; ch < nch; ch++) {
        /* decode channel */
        sbr_qmf_analysis(ac->fdsp, &sbr->mdct_ana, &sbr->dsp, ch ? R : L,
    ...
    if (ac->oc[1].m4ac.ps == 1) {
        if (sbr->ps.start) {
            AAC_RENAME(ff_ps_apply)(ac->avctx, &sbr->ps, sbr->X[0], sbr->X[1],
                                    sbr->kx[1] + sbr->m[1]);
        } else {

    ...
    for (ch = 0; ch < chans; ch++) {
        ...
        for (w = 0; w < ics->num_windows; w += ics->group_len[w]) {
            for (w2 = 0; w2 < ics->group_len[w]; w2++) {
                for (cmaxsfb = ics->num_swb;
                     cmaxsfb > 0 && cpe->ch[ch].zeroes[w * 16 + cmaxsfb - 1]; cmaxsfb--);
        ...
        for (w2 = 0; w2 < ics->group_len[w]; w2++) {
            ...
            for (g = 0; g < ics->num_swb; g++) {
                ...
                for (i = 0; i < ics->swb_sizes[g]; i++) {
                    float sum = (cpe->ch[0].coeffs[start + i] + p * cpe->ch[1].coeffs[start + i]) *
                                scale;
...
static void copy_input_samples(AACEncContext *s, const AVFrame *frame) {
    ...
    for (ch = 0; ch < s->channels; ch++) {
        ...
        memcpy(&s->planar_samples[ch][1024], &s->planar_samples[ch][2048],
               1024 * sizeof(s->planar_samples[0][0]));
...
static int aac_encode_frame(AVCodecContext *avctx, AVPacket *avpkt,
    ...
    for (i = 0; i < s->chan_map[0]; i++) {
        ...
        for (ch = 0; ch < chans; ch++) {
            ...
            for (k = 0; k < 1024; k++) {
                if (!(fabs(cpe->ch[ch].coeffs[k]) <
                      1E16)) { // Ensure headroom for energy calculation
...
static av_cold int alloc_buffers(AVCodecContext *avctx, AACEncContext *s) {
    int ch;
    FF_ALLOCZ_ARRAY_OR_GOTO(avctx, s->buffer.samples, s->channels,
                            3 * 1024 * sizeof(s->buffer.samples[0]), alloc_fail);
...
static av_cold int aac_encode_init(AVCodecContext *avctx) {
    ...
    WARN_IF(1024.0 * avctx->bit_rate / avctx->sample_rate > 6144 * s->channels,
            ...
            6144 * s->channels);
    avctx->bit_rate = (int64_t)
    FFMIN(6144 * s->channels / 1024.0 * avctx->sample_rate,
          avctx->bit_rate);
...
static const AVOption aacenc_options[] = {
        {"aac_coder", "Coding algorithm",              offsetof(AACEncContext,
                                                                options.coder),            AV_OPT_TYPE_INT,   {.i64 = AAC_CODER_TWOLOOP}, 0,
                                                                                                                                                   AAC_CODER_NB -
                                                                                                                                                   1,
                AACENC_FLAGS, "coder"},
        {"anmr",      "ANMR method",                   0,                                  AV_OPT_TYPE_CONST, {.i64 = AAC_CODER_ANMR},    INT_MIN, INT_MAX,
                AACENC_FLAGS, "coder"},
        {"twoloop",   "Two loop searching method",     0,                                  AV_OPT_TYPE_CONST, {.i64 = AAC_CODER_TWOLOOP}, INT_MIN, INT_MAX,
                AACENC_FLAGS, "coder"},
        {"fast",      "Constant quantizer",            0,                                  AV_OPT_TYPE_CONST, {.i64 = AAC_CODER_FAST},    INT_MIN, INT_MAX,
                AACENC_FLAGS, "coder"},
        {"aac_ms",    "Force M/S stereo coding",       offsetof(AACEncContext,
                                                                options.mid_side),         AV_OPT_TYPE_BOOL,  {.i64 = -1},                -1,      1,
                AACENC_FLAGS},
        {"aac_is",    "Intensity stereo coding",       offsetof(AACEncContext,
                                                                options.intensity_stereo), AV_OPT_TYPE_BOOL,  {.i64 = 1},                 -1,      1,
                AACENC_FLAGS},
        {"aac_pns",   "Perceptual noise substitution", offsetof(AACEncContext,
                                                                options.pns),              AV_OPT_TYPE_BOOL,  {.i64 = 1},                 -1,      1,
                AACENC_FLAGS},
        {"aac_tns",   "Temporal noise shaping",        offsetof(AACEncContext,
                                                                options.tns),              AV_OPT_TYPE_BOOL,  {.i64 = 1},                 -1,      1,
                AACENC_FLAGS},
        {"aac_ltp",   "Long term prediction",          offsetof(AACEncContext,
                                                                options.ltp),              AV_OPT_TYPE_BOOL,  {.i64 = 0},                 -1,      1,
                AACENC_FLAGS},
        {"aac_pred",  "AAC-Main prediction",           offsetof(AACEncContext,
                                                                options.pred),             AV_OPT_TYPE_BOOL,  {.i64 = 0},                 -1,      1,
                AACENC_FLAGS},
        {NULL}
};

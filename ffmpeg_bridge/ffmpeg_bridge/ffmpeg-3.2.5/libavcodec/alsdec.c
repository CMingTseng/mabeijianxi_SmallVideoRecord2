    ...
    if (sconf->bgmc) {
        ...
        for (sb = 0; sb < sub_blocks; sb++) {
            ...
            ff_bgmc_decode(gb, sb_len, current_res,
                           delta[sb], sx[sb], &high, &low, &value, ctx->bgmc_lut,
                           ctx->bgmc_lut_status);
    ...
    for (dep = 0; !ch[dep].stop_flag; dep++) {
        ...
        if (ch[dep].time_diff_flag) {
            ...
            if (ch[dep].time_diff_sign) {
                ...
                if (end < t) {
                    av_log(ctx->avctx, AV_LOG_ERROR, "end %"
                    PTRDIFF_SPECIFIER
                    " smaller than time diff index %d.\n", end, t);
                    return AVERROR_INVALIDDATA;
                }
...
static SoftFloat_IEEE754 multiply(SoftFloat_IEEE754 a, SoftFloat_IEEE754 b) {
    ...
    if (cutoff_bit_count > 0) {
        last_2_bits = (unsigned char) (((unsigned int) mantissa_temp >> (cutoff_bit_count - 1)) &
                                       0x3);
        if ((last_2_bits == 0x3) || ((last_2_bits == 0x1) && ((unsigned int) mantissa_temp &
                                                              ((0x1UL << (cutoff_bit_count - 1)) -
                                                               1)))) {
            // Need to round up
            mantissa_temp += (uint64_t)
            0x1 << cutoff_bit_count;
        }
    }
...
static int decode_frame(AVCodecContext *avctx, void *data, int *got_frame_ptr,
    ...
    if (sconf->samples != 0xFFFFFFFF)
        ctx->cur_frame_length = FFMIN(sconf->samples - ctx->frame_id * (uint64_t)
    sconf->frame_length,
            sconf->frame_length);
    else
    ctx->cur_frame_length = sconf->frame_length;

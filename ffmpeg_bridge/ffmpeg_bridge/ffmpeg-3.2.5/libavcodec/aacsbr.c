            ...
            for (e = 1; e <= sbr->data[ch].bs_num_env; e++)
                for (k = 0; k < sbr->n[sbr->data[ch].bs_freq_res[e]]; k++) {
                    if (sbr->data[ch].bs_amp_res)
                        sbr->data[ch].env_facs[e][k] = ff_exp2fi(
                                sbr->data[ch].env_facs_q[e][k] + 6);
                    else
    ...
    for (e = 0; e < ch_data->bs_num_env; e++) {
        ...
        for (k = 0; k < sbr->n_lim; k++) {
            ...
            for (m = sbr->f_tablelim[k] - sbr->kx[1];
                 m < sbr->f_tablelim[k + 1] - sbr->kx[1]; m++) {
            ...
            gain_max = limgain[sbr->bs_limiter_gains] *
...
static void sbr_hf_assemble(float Y1[38][64][2],
    ...
    if (sbr->reset) {
        for (i = 0; i < h_SL; i++) {
            memcpy(g_temp[i + 2 * ch_data->t_env[0]], sbr->gain[0],
                   m_max * sizeof(sbr->gain[0][0]));

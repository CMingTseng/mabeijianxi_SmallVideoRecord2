...
typedef struct AMRWBContext {
    ...
    float excitation_buf[AMRWB_P_DELAY_MAX + LP_ORDER + 2 +
                         AMRWB_SFR_SIZE]; ///< current excitation and all necessary excitation history
    ...
    float samples_az[LP_ORDER +
                     AMRWB_SFR_SIZE];         ///< low-band samples and memory from synthesis at 12.8kHz
    float samples_up[UPS_MEM_SIZE +
                     AMRWB_SFR_SIZE];     ///< low-band samples and memory processed for upsampling
    float samples_hb[LP_ORDER_16k +
                     AMRWB_SFR_SIZE_16k]; ///< high-band samples and memory from synthesis at 16kHz
...
static int amrwb_decode_frame(AVCodecContext *avctx, void *data,
    ...
    for (sub = 0; sub < 4; sub++) {
        ...
        ctx->acelpf_ctx.acelp_apply_order_2_transfer_function(&ctx->samples_up[UPS_MEM_SIZE],
                                                              ...
                                                              hpf_zeros, hpf_31_poles,
                                                              hpf_31_gain, ctx->hpf_31_mem,
                                                              AMRWB_SFR_SIZE);
        ...
        ctx->acelpf_ctx.acelp_apply_order_2_transfer_function(hb_samples,
                                                              &ctx->samples_up[UPS_MEM_SIZE],
                                                              hpf_zeros, hpf_400_poles,
                                                              hpf_400_gain, ctx->hpf_400_mem,
                                                              AMRWB_SFR_SIZE);

    ...
    for (ch = !s->cpl_on; ch <= s->fbw_channels; ch++) {
        ...
        for (blk = 1; blk < s->num_blocks; blk++, exp += AC3_MAX_COEFS) {
            ...
            if (ch == CPL_CH && exp_diff > (EXP_DIFF_THRESHOLD *
                                            (s->blocks[blk].end_freq[ch] - s->start_freq[ch]) /
                                            AC3_MAX_COEFS))
                exp_strategy[blk] = EXP_NEW;
...
void ff_ac3_group_exponents(AC3EncodeContext *s) {
    ...
    for (blk = 0; blk < s->num_blocks; blk++) {
        ...
        for (ch = !block->cpl_in_use; ch <= s->channels; ch++) {
            ...
            nb_groups = exponent_group_tab[cpl][exp_strategy - 1][block->end_freq[ch] -
                                                                  s->start_freq[ch]];
...
static void output_audio_block(AC3EncodeContext *s, int blk) {
    ...
    for (ch = !block->cpl_in_use; ch <= s->channels; ch++) {
        ...
        /* exponent groups */
        nb_groups = exponent_group_tab[cpl][s->exp_strategy[ch][blk] - 1][block->end_freq[ch] -
                                                                          s->start_freq[ch]];
        for (i = 1; i <= nb_groups; i++)
...
static av_cold void set_bandwidth(AC3EncodeContext *s) {
    ...
    if (s->cutoff) {
        ...
        s->bandwidth_code = ac3_bandwidth_tab[s->fbw_channels - 1][s->bit_alloc.sr_code][
                s->frame_size_code / 2];
    }
    ...
    if (s->cpl_enabled) {
        if (s->options.cpl_start != AC3ENC_OPT_AUTO) {
            ...
            cpl_start = ac3_coupling_start_tab[s->channel_mode - 2][s->bit_alloc.sr_code][
                    s->frame_size_code / 2];
            if (cpl_start < 0) {

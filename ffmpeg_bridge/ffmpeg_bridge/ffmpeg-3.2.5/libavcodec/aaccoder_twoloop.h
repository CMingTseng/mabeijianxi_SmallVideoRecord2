...
static void search_for_quantizers_twoloop(AVCodecContext *avctx,
    ...
    do {
        ...
        do {
            ...
            for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
                ...
                for (g = 0; g < sce->ics.num_swb; g++) {
                    ...
                    if (prev != -1) {
                        int sfdiff = av_clip(sce->sf_idx[w * 16 + g] - prev + SCALE_DIFF_ZERO, 0,
                                             2 * SCALE_MAX_DIFF);
        ...
        for (i = 0; i < 2 && (overdist || recomprd); ++i) {
            if (recomprd) {
                ...
                for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
                    ...
                    for (g = 0; g < sce->ics.num_swb; g++) {
                        ...
                        qenergies[w * 16 + g] = qenergy;
                        if (prev != -1) {
                            int sfdiff = av_clip(sce->sf_idx[w * 16 + g] - prev + SCALE_DIFF_ZERO,
                                                 0, 2 * SCALE_MAX_DIFF);
                            bits += ff_aac_scalefactor_bits[sfdiff];
                        }
                        tbits += bits;
            ...
            if (!i && s->options.pns && its > maxits / 2 && tbits > toofewbits) {
                ...
                if (overdist) {
                    ...
                    for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
                        for (g = start = 0;
                             g < sce->ics.num_swb; start += sce->ics.swb_sizes[g++]) {
                            if (start >= pns_start_pos && !sce->zeroes[w * 16 + g] &&
                                sce->can_pns[w * 16 + g]) {
                    ...
                    zspread = FFMIN3(min_spread_thr_r * 8.f, zspread,
                                     ((toomanybits - tbits) * min_spread_thr_r +
                                      (tbits - toofewbits) * max_spread_thr_r) /
                                     (toomanybits - toofewbits + 1));
                    maxzeroed = FFMIN(zeroable,
                    ...
                    for (zloop = 0; zloop < 2; zloop++) {
                        ...
                        for (g = sce->ics.num_swb - 1; g > 0 && zeroed < maxzeroed; g--) {
                            if (sce->ics.swb_offset[g] < pns_start_pos)
                                continue;
                            for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
                                if (!sce->zeroes[w * 16 + g] && sce->can_pns[w * 16 + g] &&
                                    spread_thr_r[w * 16 + g] <= zspread
                                    && sce->sf_idx[w * 16 + g] > loopminsf
                                    && (dists[w * 16 + g] > loopovrfactor * uplims[w * 16 + g] ||
                                        !(mcb = find_min_book(maxvals[w * 16 + g],
                                                              sce->sf_idx[w * 16 + g]))
                                        || (mcb <= 1 && dists[w * 16 + g] >
                                                        FFMIN(uplims[w * 16 + g],
                                                              euplims[w * 16 + g])))) {
        ...
        minscaler = nminscaler = av_clip(minscaler, SCALE_ONE_POS - SCALE_DIV_512,
                                         SCALE_MAX_POS - SCALE_DIV_512);
        prev = -1;
        for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
                ...
                if (!sce->zeroes[w * 16 + g]) {
                    ...
                    if ((!cmb || dists[w * 16 + g] > uplims[w * 16 + g]) &&
                        ...
                        for (i = 0; i < edepth && sce->sf_idx[w * 16 + g] > mindeltasf; ++i) {
                            int cb, bits;
                            float dist, qenergy;
                            int mb = find_min_book(maxvals[w * 16 + g],
                                                   sce->sf_idx[w * 16 + g] - 1);
                            cb = find_min_book(maxvals[w * 16 + g], sce->sf_idx[w * 16 + g]);
                            dist = qenergy = 0.f;
                            bits = 0;
                            if (!cb) {
                                maxsf[w * 16 + g] = FFMIN(sce->sf_idx[w * 16 + g] - 1,
                                                          maxsf[w * 16 + g]);
                             ...
                             */
                            if (!g && sce->ics.num_windows > 1 &&
                                dists[w * 16 + g] >= euplims[w * 16 + g])
                                maxsf[w * 16 + g] = FFMIN(sce->sf_idx[w * 16 + g],
                                                          maxsf[w * 16 + g]);
                            for (w2 = 0; w2 < sce->ics.group_len[w]; w2++) {
                                int b;
                                float sqenergy;
                            ...
                            if (mb && (sce->sf_idx[w * 16 + g] < mindeltasf || (
                                    ...
                                    && (fabsf(qenergies[w * 16 + g] - energies[w * 16 + g]) <
                                        euplims[w * 16 + g])
                            ))) {
                                break;
                            }
                               ...
                               sce->sf_idx[w * 16 + g] < FFMIN(maxdeltasf, maxsf[w * 16 + g])
                               &&
                    ...
                    prev = sce->sf_idx[w * 16 + g] = av_clip(sce->sf_idx[w * 16 + g], mindeltasf,
                                                             maxdeltasf);
            ...
            for (g = 0; g < sce->ics.num_swb; g++) {
                if (!sce->zeroes[w * 16 + g]) {
                    ...
                    sce->sf_idx[w * 16 + g] = av_clip(sce->sf_idx[w * 16 + g],
                                                      prev - SCALE_MAX_DIFF, prev + SCALE_MAX_DIFF);
                    sce->band_type[w * 16 + g] = find_min_book(maxvals[w * 16 + g],
                                                               sce->sf_idx[w * 16 + g]);
                    prev = sce->sf_idx[w * 16 + g];

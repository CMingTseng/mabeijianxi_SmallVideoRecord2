    ...
    for (i = 0; i < avctx->channels; i++) {
        ...
        if (avctx->flags & AV_CODEC_FLAG_QSCALE)
            ...
            pch->attack_threshold = lame_calc_attack_threshold(
                    avctx->bit_rate / avctx->channels / 1000);
...
static av_cold int psy_3gpp_init(FFPsyContext *ctx) {
    ...
    int chan_bitrate = ctx->avctx->bit_rate /
                       ((ctx->avctx->flags & CODEC_FLAG_QSCALE) ? 2.0f : ctx->avctx->channels);
    ...
    if (ctx->avctx->flags & CODEC_FLAG_QSCALE) {
        ...
        chan_bitrate = (int) (chan_bitrate / 120.0 *
                              (ctx->avctx->global_quality ? ctx->avctx->global_quality : 120));
    }
    ...
    for (j = 0; j < 2; j++) {
        ...
        float en_spread_hi = (j || (chan_bitrate <= 22.0f)) ? PSY_3GPP_EN_SPREAD_HI_S
 ...
 */
static av_unused FFPsyWindowInfo
...
pch->
win_energy = pch->win_energy * 7 / 8 + sum2 / 64;
...
pch->
next_grouping = window_grouping[attack_n];
pch->
next_window_seq = next_type;
} else {
for (
i = 0;
i < 3; i++)
...
static int calc_bit_demand(AacPsyContext *ctx, float pe, int bits, int size,
     ...
     */
    bit_factor = 1.0f - bit_save + ((bit_spend - bit_save) / (ctx->pe.max - ctx->pe.min)) *
...
static void psy_hp_filter(const float *firbuf, float *hpfsmpl, const float *psy_fir_coeffs) {
    ...
    for (i = 0; i < AAC_BLOCK_SIZE_LONG; i++) {
        ...
        for (j = 0; j < ((PSY_LAME_FIR_LEN - 1) / 2) - 1; j += 2) {
            ...
            sum2 += psy_fir_coeffs[j + 1] *
                    (firbuf[i + j + 1] + firbuf[i + PSY_LAME_FIR_LEN - j - 1]);
        }
...
static void psy_3gpp_analyze_channel(FFPsyContext *ctx, int channel,
    ...
    float pe = pctx->chan_bitrate > 32000 ? 0.0f : FFMAX(50.0f, 100.0f -
                                                                pctx->chan_bitrate * 100.0f /
                                                                32000.0f);
    const int num_bands = ctx->num_bands[wi->num_windows == 8];
    const uint8_t *band_sizes = ctx->bands[wi->num_windows == 8];
    AacPsyCoeffs *coeffs = pctx->psy_coef[wi->num_windows == 8];
    const float avoid_hole_thr =
            wi->num_windows == 8 ? PSY_3GPP_AH_THR_SHORT : PSY_3GPP_AH_THR_LONG;
    const int bandwidth = ctx->cutoff ? ctx->cutoff : AAC_CUTOFF(ctx->avctx);
    ...
    for (w = 0; w < wi->num_windows * 16; w += 16) {
        ...
        for (g = 1; g < num_bands; g++) {
            ...
            spread_en[w + g] = FFMAX(bands[g].energy,
                                     spread_en[w + g - 1] * coeffs[g].spread_hi[1]);
        }
        for (g = num_bands - 2; g >= 0; g--) {
            bands[g].thr = FFMAX(bands[g].thr, bands[g + 1].thr * coeffs[g].spread_low[0]);
            spread_en[w + g] = FFMAX(spread_en[w + g],
                                     spread_en[w + g + 1] * coeffs[g].spread_low[1]);
        }
        ...
        for (g = 0; g < num_bands; g++) {
            ...
            if (!(wi->window_type[0] == LONG_STOP_SEQUENCE ||
                  (!w && wi->window_type[1] == LONG_START_SEQUENCE)))
    ...
    if (ctx->avctx->flags & CODEC_FLAG_QSCALE) {
         ...
         */
        desired_pe = pe * (ctx->avctx->global_quality ? ctx->avctx->global_quality : 120) /
                     (2 * 2.5f * 120.0f);
        ...
        desired_bits = calc_bit_demand(pctx, pe, ctx->bitres.bits, ctx->bitres.size,
                                       wi->num_windows == 8);
...
static FFPsyWindowInfo psy_lame_window(FFPsyContext *ctx, const float *audio,
    ...
    if (la) {
        ...
        for (i = 0; i < PSY_LAME_NUM_SUBBLOCKS; i++) {
            energy_subshort[i] = pch->prev_energy_subshort[i + ((AAC_NUM_BLOCKS_SHORT - 1) *
                                                                PSY_LAME_NUM_SUBBLOCKS)];
            assert(pch->prev_energy_subshort[i +
                                             ((AAC_NUM_BLOCKS_SHORT - 2) * PSY_LAME_NUM_SUBBLOCKS +
                                              1)] > 0);
            attack_intensity[i] = energy_subshort[i] / pch->prev_energy_subshort[i +
        ...
        for (i = 0; i < AAC_NUM_BLOCKS_SHORT * PSY_LAME_NUM_SUBBLOCKS; i++) {
            const float *const pfe =
                    pf + AAC_BLOCK_SIZE_LONG / (AAC_NUM_BLOCKS_SHORT * PSY_LAME_NUM_SUBBLOCKS);
            float p = 1.0f;
            for (; pf < pfe; pf++)
                p = FFMAX(p, fabsf(*pf));

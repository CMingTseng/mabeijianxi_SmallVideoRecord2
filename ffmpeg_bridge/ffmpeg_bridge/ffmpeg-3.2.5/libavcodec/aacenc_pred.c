...
void ff_aac_search_for_pred(AACEncContext *s, SingleChannelElement *sce) {
    ...
    for (sfb = PRED_SFB_START; sfb < pmax; sfb++) {
        ...
        dist1 = quantize_and_encode_band_cost(s, NULL, &sce->coeffs[start_coef], NULL,
                                              ...
                                              cb_n, s->lambda / band->threshold, INFINITY, &cost1,
                                              NULL, 0);
        cost_coeffs += cost1;
        ...
        if (cb_n < RESERVED_BT)
            cb_p = av_clip(find_min_book(find_max_val(1, num_coeffs, S34), sce->sf_idx[sfb]),
                           cb_min, cb_max);
        else
            cb_p = cb_n;
        ...
        for (i = 0; i < num_coeffs; i++)
            sce->prcoeffs[start_coef + i] +=
                    QERR[i] != 0.0f ? (sce->prcoeffs[start_coef + i] - QERR[i]) : 0.0f;
        s->abs_pow34(P34, &sce->prcoeffs[start_coef], num_coeffs);
        if (cb_n < RESERVED_BT)
            cb_p = av_clip(find_min_book(find_max_val(1, num_coeffs, P34), sce->sf_idx[sfb]),
                           cb_min, cb_max);
        else
            cb_p = cb_n;
        dist2 = quantize_and_encode_band_cost(s, NULL, &sce->prcoeffs[start_coef], NULL,
                                              P34, num_coeffs, sce->sf_idx[sfb],
                                              cb_p, s->lambda / band->threshold, INFINITY, NULL,
                                              NULL, 0);
        for (i = 0; i < num_coeffs; i++)
